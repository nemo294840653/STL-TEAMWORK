...
template<class T>
node<T>::node(T val, node<T> *preobj,  node<T> *nextobj)
{
	value = val;
	pre = preobj;
	next=nextobj;
...
template<class T>
class tiny_list {
	friend ostream & operator<<(ostream &out, tiny_list<T> &obj)
	{
...
public:
	...
	tiny_list& operator=(const tiny_list<T> &obj);
...
//清除所有元素
template<class T>
void tiny_list<T>::clear()
{
	while (_size > 0) remove(head->next);
}
//删除某个结点，并返回已删除结点的值
template<class T>
T tiny_list<T>::remove(node<T> *obj)
{
	T temp = obj->value;
...
//构造函数     
template<class T>
tiny_list<T>::tiny_list()
{
	init();
}
template<class T>
tiny_list<T>::tiny_list(int n)
{
	init();
	head->insertAsnext(n);
	_size++;
...
template<class T>
tiny_list<T>::tiny_list(int num, T value)
{
	init();
	node<T>* temp = head;
...
template<class T>
tiny_list<T>::tiny_list( tiny_list<T> const &obj)
{
	copy_node(obj.first(), obj._size);
}
template<class T>
tiny_list<T>::tiny_list(const tiny_list<T> &L, int r, int n)
{
	copy_node(L[r], n);
}
/*     析构函数      */
...
template<class T>
void tiny_list<T>::copy_node(node<T> *start,int n)
{
	init();
	while (n--)
	{
...
//重载[]操作符，返回当前索引的结点的值
template<class T>
T& tiny_list<T>::operator[] (int index) 
{
	node<T> *p = first();
	while (index--)
	{
...
template<class T>
tiny_list<T>& tiny_list<T>::operator=(const tiny_list<T> &obj)
{
	_size = obj._size;
	clear();
	int temp= obj._size;
...
//返回头结点之后的第一个元素结点
template<class T>
node<T>* tiny_list<T>::first()const
{
	return head->next;
}
//在列表头部插入元素(会重新分配内存)
template <typename T>
node<T>* tiny_list<T>::insertAsFirst(T const& e) {
	_size++;
...
template <typename T>
node<T>* tiny_list<T>::search(T const& e)
{
...
{
	tiny_list<T> *temp=new tiny_list<T>;
