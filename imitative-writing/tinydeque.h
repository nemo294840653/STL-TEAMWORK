...
功能清单：
tiny_deque()
tiny_deque(const tiny_deque& obj)
void clear()
...
public:
	friend bool operator == (const tiny_deque& lhs, const tiny_deque& rhs) {
...
public:
	...
	public:
		...
		typedef typename tiny_deque::value_type      value_type;
		typedef typename tiny_deque::difference_type difference_type;
		typedef typename tiny_deque::pointer         pointer;
		typedef typename tiny_deque::reference       reference;
	...
	public:
	 ...
	 public:
		 ...
		 public:
			 ...
			 typedef typename tiny_deque::value_type      value_type;
			 typedef typename tiny_deque::difference_type difference_type;
			 typedef typename tiny_deque::const_pointer   pointer;
			 typedef typename tiny_deque::const_reference reference;
...
//	allocator <U> = allocator <T>::Rebind <U>::other.
 ...
 private:
		...
		public:
			explicit tiny_deque(const allocator_type& a = allocator_type()) : _a(a)
			{
				size_num = 0;
	...
	//			end_iterator = uninitialized_copy(_a, obj.begin_iterator, obj.end_iterator, begin_iterator);
			...
			tiny_deque& operator = (const tiny_deque& obj)
			{
				if (this == &obj) 
				{
					return *this;
	...
	//				end_iterator = uninitialized_copy(_a, obj.begin_iterator, obj.end_iterator, begin_iterator);
			...
			const_reference operator [] (size_type index) const {
				return const_cast<tiny_deque*>(this)->operator[](index);
			}
			...
			const_reference at(size_type index) const {
				return const_cast<tiny_deque*>(this)->at(index);
			}
			reference back() {
			...
			const_reference back() const {
				return const_cast<tiny_deque*>(this)->back();
			}
			...
			const_reference front() const {
				return const_cast<tiny_deque*>(this)->front();
			}
...
typename std::size_t tiny_deque<T, A>::block_size = 10;
